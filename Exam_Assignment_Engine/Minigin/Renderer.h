...
namespace dae
{
	...
	public:
		...
		void RenderTexture(std::shared_ptr<SDL_Texture> texture, const float x, const float y, const float w, const float h, float angleInDegrees, const SDL_Point& rotationCenter, const SDL_RendererFlip& flip = SDL_RendererFlip::SDL_FLIP_NONE) const;
